...
function initTable(table, data, type, typeField)
{
    ...
    data.forEach((item) => {
        ...
        row.push(item.sum_fee_pasc);
        row.push(item.avg_fee_pasc);
        allData.push(row);
    });
...
function initChart(ctx, data, type, typeField)
{
    ...
    data.forEach((item) => {
        ...
        datasets.sum.data.push(item.sum_fee_pasc);
        datasets.avg.data.push(item.avg_fee_pasc);
    });
